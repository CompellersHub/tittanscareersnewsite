...
export const MFAManagement = () => {
  ...
  const { data: userData, isLoading } = useFetchAuthUser();
  
  const [showEnrollment, setShowEnrollment] = useState(false);

  // Extract MFA info from user data
  const mfaEnabled = userData?.data?.mfa_enabled ?? false;
  const mfaFactors = userData?.data?.mfa_factors ?? [];
  const hasMFA = mfaEnabled && mfaFactors.length > 0;

  // Mutation to disable MFA
  const disableMFAMutation = useMutation({
    mutationFn: async () => {
      const response = await authApi.post("/auth/mfa/disable");
